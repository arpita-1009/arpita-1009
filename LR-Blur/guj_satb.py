...
def Land_use(lt):
    ...
    for i,j in enumerate(lt):
        if 'જમીનનો ઉપયોગ' in j or 'થા૧વં ઇડ૯' in j :
            if len(lt[i+1])>=1:
                return lt[i+1]
            elif len(lt[i+2])>=1:
                return lt[i+2]
...
def pdf_to_text(path):
    ...
    for i in range(len(images)):
        text_g = tesserocr.image_to_text(images[i],lang='guj')
    # pytesseract.pytesseract.tesseract_cmd = r'C:\Users\Arpita Patel\AppData\Local\Tesseract-OCR\tesseract.exe'
    # images = convert_from_path(path,260,poppler_path=r'C:\Users\Arpita Patel\Documents\GitHub\poppler-0.68.0\bin')
    # for i in range(len(images)):
    #     text_g = pytesseract.image_to_string(images[i] , lang='guj+eng')
        #text_e = tesserocr.image_to_text(images[i],lang='eng+guj')
        texts_guj.append(text_g)
        #texts_eng.append(text_e)
...
#     return [district(guj),taluka(guj),village(guj , index),survey(guj),area(guj),'',', '.join(ow),', '.join(boja(guj, index))]

def fields(guj_txt, eng_txt):
    guj, eng = guj_txt, eng_txt
    ow, index = owner(guj)
    ta, index1 = taluka(guj)
    return [district(guj),ta,village(guj, index1),survey(eng),area(guj),'બોજા અને બીજા હક્ક ની વિગતો',','.join(ow),'',pdf_date(eng),Land_use(eng)]
